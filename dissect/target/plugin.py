...
def plugins(
    ...
    happen as stated in the bove cases.
    ...
    if osfilter is not None:
        ...
        # directory (dissect.target.plugins) for the build in plugins. For the
        # plugins in the directory specified in --plugin-path, the hierarchy
        # starts at that directory.
        #
        # Build-in OSPlugins do have the dissect.target.plugins path in their
    ...
    def _walk(
        ...
        for key, obj in root.items():
            ...
            # A branch in the PLUGINS tree is traveresed to the next level if
            # there are no filters (which in effect means all plugins are
            # returned).
            # If there is a filter, a branch is traversed if there is no `_os`
            # item on the next level. If there there is an `_os` item, it is
            # only traversed into if the module_path matches in full the
            # (beginning of) the filter_path.
            if (
                not filter_path
                or filter_path
                and (
                    issubclass(osfilter, general.default.DefaultPlugin)
                    or "_os" not in obj
                    or ("_os" in obj and (module_path == filter_path or module_path == filter_path[: len(module_path)]))
                )
            ):
                if key.startswith("_"):
                    if key in special_keys:
                        # OSPlugins are treated special and are only retruned
...
def plugin_function_index(target: Target) -> tuple[dict[str, PluginDescriptor], set[str]]:
    ...
    if target._os_plugin is None:
        ...
        raise TypeError("target._os_plugin must be either None, a subclass of OSPlugin or an instance of OSPlugin")
    ...
    for available_original in all_plugins:
        ...
        for exported in available["exports"]:
            if exported == "get_all_records":
                continue
            if issubclass(available["cls"], OSPlugin) and os_type == general.default.DefaultPlugin:
                # This makes the os plugin exports listed under the special 'OS plugins' header.
                available["module"] = ""
